...
# w1_visited_points = Hash.new
# w2_visited_points = Hash.new
  ...
  wire_steps.each do |s|
    if s=~ /([LRUD]?)(\d+)/
      ...
      if direction == "L"
        ...
        ((wire_next_point[0])..(wire_current_point[0])).each do |p|
          wire_visited_points[[p, wire_next_point[1] ]] = wire_counter if wire_visited_points[[p, wire_next_point[1] ]].nil?
        end
      elsif direction == "R"
        wire_next_point[0] = wire_current_point[0] + length.to_i
        wire_next_point[1] = wire_current_point[1]
        ((wire_current_point[0])..(wire_next_point[0])).each do |p|
          wire_visited_points[[p, wire_next_point[1]]] = wire_counter if wire_visited_points[[p, wire_next_point[1]]].nil?
        end
      elsif direction == "U"
        wire_next_point[0] = wire_current_point[0]
        wire_next_point[1] = wire_current_point[1] + length.to_i
        ((wire_current_point[1])..(wire_next_point[1])).each do |p|
          wire_visited_points[[wire_current_point[0], p]] = wire_counter if wire_visited_points[[wire_current_point[0], p]].nil?
        end
      elsif direction == "D"
        wire_next_point[0] = wire_current_point[0]
        wire_next_point[1] = wire_current_point[1] - length.to_i
        ((wire_next_point[1])..(wire_current_point[1])).each do |p|
          wire_visited_points[[wire_current_point[0], p]] = wire_counter if wire_visited_points[[wire_current_point[0], p]].nil?
        end
      end
    end
